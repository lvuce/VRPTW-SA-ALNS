...
def read_data():
    ...
    # 使用 'with' 确保文件在读取后自动关闭
    with open(file_path, 'r') as data:
        line_count = 0
        for line in data:
            line_count += 1

            if line_count < 10 or line_count > 80 :
                continue

            # 去除行尾的空白字符，并按空白字符（空格或制表符）分割
            cur_line = line.strip().split()

            # 创建一个新的 Node 对象并赋值
            node = Node()
            try:
                node.name = int(cur_line[0])  # 节点名称
                node.x = int(cur_line[1])  # x 坐标
                node.y = int(cur_line[2])  # y 坐标
                node.demand = int(cur_line[3])  # 需求量
                node.ready = int(cur_line[4])  # 准备时间
                node.due = int(cur_line[5])  # 截止时间
                node.service = int(cur_line[6])  # 服务时间
            except ValueError as e:
                # 如果转换为整数失败，跳过当前行并打印错误
                print(f"跳过第 {line_count} 行，错误信息: {e}")
                continue

            # 将解析后的节点添加到 Sol 对象的节点列表中
